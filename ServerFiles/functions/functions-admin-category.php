...
function renderCategoryTable($host, $dbname, $user, $password)
{
    ...
    <table class="table table-striped table-bordered">
        <div style="text-align: right; padding: 15px;">
            <a href="?cat=category&action=add" class="btn btn-primary"><i class="bi bi-file-plus"></i> Add Category</a>
        </div>
        ...
        <tbody>
            ...
            foreach ($categories as $category) {
                ...
                echo "<a href='?cat=category&action=edit&categoryId=" . $category['CategoryID'] . "'class='btn btn-primary'><i class='bi bi-pencil-square'></i> Edit</a>"; // Link to edit
                echo "</td>";
                echo "<td class='align-middle' style='width: 10%'>";
                echo "<a href='?cat=category&action=delete&categoryId=" . $category['CategoryID'] . "'class='btn btn-danger'><i class='bi bi-trash'></i> Delete</a>"; // Link to delete
                echo "</td>";
                echo "</tr>";
            }
            ?>
...
function renderEditCategoryForm($host, $dbname, $user, $password, $categoryId = null)
{
    ...
    // Fetch category information if categoryId is provided
    if ($categoryId) {
        $categories = $categoryClass->getCategories();
        $category = null;
    ...
    <div class="container mt-4">
        ...
        <form method="post" action="">
            <div class="mb-3">
                ...
                <input type="number" id="categoryId" name="categoryId" class="form-control"
                    value="<?php echo htmlspecialchars($categoryId); ?>" required>
            </div>
