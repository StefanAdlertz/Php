...
function renderProductTable($host, $dbname, $user, $password)
{
    ...
    <table class="table table-striped table-bordered">
        <div style="text-align: right; padding: 15px;">
            <a href="?cat=product&action=add" class="btn btn-primary"><i class="bi bi-file-plus"></i>
                Add Product</a>
        </div>
        ...
        <tbody>
            ...
            foreach ($products as $product) {
                ...
                echo "<a href='?cat=product&action=edit&productId=" . $product['ProductID'] . "'class='btn btn-primary'><i class='bi bi-pencil-square'></i> Edit</a>";
                echo "</td>";
                echo "<td class='align-middle' style='width: 10%'>";
                echo "<a href='?cat=product&action=delete&productId=" . $product['ProductID'] . "'class='btn btn-danger'><i class='bi bi-trash'></i> Delete</a>";
                echo "</td>";
                echo "</tr>";
            }
            ?>
...
function renderAddProductForm($host, $dbname, $user, $password)
{
    ...
    // Create an instance of the Category class for the List categoryId in the form
...
function renderEditProductForm($host, $dbname, $user, $password, $productId = null)
{
    ...
    // Fetch product information if productId is provided
    $product = null;
    if ($productId) {
        $products = $productClass->getProducts();
    ...
    <div class="container mt-4">
        ...
        <form method="post" action="">
            <div class="mb-3">
                ...
                <input type="number" id="productId" name="productId" class="form-control"
                    value="<?php echo htmlspecialchars($productId); ?>" required>
            </div>
